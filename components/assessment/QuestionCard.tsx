  ...
  return (
    <div
      className={`rounded-xl border bg-white transition-all duration-200 ${
        expanded ? "border-orange-300 shadow-md" : "border-neutral-200 hover:border-orange-200 hover:shadow-sm"
      }`}
    >
      <button type="button" onClick={onToggle} className="flex w-full items-start gap-3 px-3 py-3 text-left">
        <span
          className={`mt-0.5 flex min-w-6 shrink-0 items-center justify-center rounded-full px-1.5 py-0.5 text-xs font-semibold transition-colors ${
            expanded ? "bg-gradient-to-br from-orange-500 to-amber-400 text-white" : "bg-neutral-100 text-neutral-600"
          }`}
        >
          {question.number}
        </span>
