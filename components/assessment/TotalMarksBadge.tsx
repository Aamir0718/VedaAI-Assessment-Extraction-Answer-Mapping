...
type Props = { questions: Question[]; evaluations: Evaluation[] };

/** The whole-paper score — only meaningful (and only shown) once grading has run. */
export function TotalMarksBadge({ questions, evaluations }: Props) {
  if (evaluations.length === 0) return null;

  const { earned, possible } = computeTotalMarks(questions, evaluations);
